...
/*  var filterFeatures = function (it) {
  return (it.offer.features === filterParams.features) || (filterParams.features === []);
};  */
...
var resultFilter = function (it) {
  return filterType(it) && filterRooms(it) && filterGuests(it) && filterPrice(it);/*  && filterFeatures(it) */
};
